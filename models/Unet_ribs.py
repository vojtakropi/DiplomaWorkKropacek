...
def UNet_ribs():
    ...
    # c5, p5 = down_block(p4, f[3])
    # c6, p6 = down_block(p5, f[3])

    bn = bottleneck(p4, f[4])  ## Bottleneck

    ## Decoder
    # u1 = up_block(bn, c6, f[3])
    # u2 = up_block(u1, c5, f[3])
    u3 = up_block(bn, c4, f[3])
    u4 = up_block(u3, c3, f[2])
    u5 = up_block(u4, c2, f[1])
    u6 = up_block(u5, c1, f[0])

    ## Last convolution with sigmoid
    outputs = Conv2D(1, 1, padding="same")(u6)

    model = Model(inputs=inputs, outputs=outputs, name='UNetRibs')
